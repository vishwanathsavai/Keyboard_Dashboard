...
def vm_forgot_password(request):
    if request.method == "GET":
        body_unicode = request.body.decode('utf-8')
        body = json.loads(body_unicode)
        print(body)
        phone = body["uid"]
        query = '''select u2.n_usrmta_token,u.b_usr_blckd   from users u , usrmta u2 where u.c_usr_ph = "''' + phone + '''" and u.n_usr_rl_id =1 and u.n_usr_id = u2.n_usrmta_usr_id  ;'''
        print(query)
        token = database_connect.RunQuery(query)
        if len(token)==0:
            result = {
...
def vm_validate_OTP(request):
    if request.method == "GET":
        ...
        try:
            phone = body["uid"]
            token = body["token"]
            query = '''select u2.n_usrmta_token  from users u , usrmta u2 where u.c_usr_ph = "''' + phone + '''" and u.n_usr_rl_id =1 and u.n_usr_id = u2.n_usrmta_usr_id  and u2.n_usrmta_token ="''' + token + '''";'''
            print(query)
            token = database_connect.RunQuery(query)
            resend = body["resend"]
            if resend == 1:
